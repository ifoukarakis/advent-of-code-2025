defmodule Dial do
  ...
  defp do_turn(dial, {:left, distance})  do
    ...
    # k = smallest positive step t such that p - t == 0 (mod 100)
    # If p == 0 the first hit is at t = 100, otherwise at t = p.
    k = if p == 0, do: 100, else: p

    count_zeros = if d < k, do: 0, else: 1 + div(d - k, 100)
    new_position = rem(rem(dial.position - distance, 100) + 100, 100)
    {new_position, count_zeros}
  end
